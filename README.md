...
à la méthode `setTimeout`). Pour les débutants en JavaScript, on pourrait imaginer qu'étant donné que le deuxième
paramètre de mes méthodes `setTimeout` est égal à 0, la méthode `veryLongTask` est appelée deux fois successivement, de manière synchrone. Vous vous trompez...
...
Dans notre exemple, après avoir exécuté la méthode `endTimer`, la stack principale est maintenant vide, et la stack secondaire contenant
...
une zone. Nous pouvons appeler à nouveau la méthode *fork* sur cette zone, afin d'avoir une hiérarchie dans nos contextes
...
Comme nous l'avons indiqué précédemment, nous pouvons avoir une hiérarchie de zones. Afin de bénéficier correctement
de cette chaîne, et pour éviter de la briser, nous devons appeler, dans chaque *hook*, l'implémentation de la zone parente. 
